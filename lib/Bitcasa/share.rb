module Bitcasa
	...
	class Share
		...
		def list
			response = @client.browse_share(self.share_key)
		...
		# @raise Bitcasa::Client::Error
		def delete
			@client.delete_share(self.share_key)
			nil
		end
		...
		def set_password(password, current_password: nil)
			response = @client.alter_share_info(self.share_key, 
		...
		def save(password: nil)
			response = @client.alter_share_info(self.share_key, 
					current_password: password, name: self.name)
			set_share_info(**response)
			nil
		end
		...
		def receive(path: nil, exists: 'RENAME')
			response = @client.receive_share(self.share_key, 
