...
module Bitcasa
	...
	class FileSystem
		...
		def copy(items, destination, exists: 'RENAME')
			raise Bitcasa::Client::InvalidArgumentError, "Invalid input, expected items" if Utils::is_blank?(items)
		...
		def list_file_versions(item)
			raise Bitcasa::Client::InvalidArgumentError, "Invalid input, expected Item or string path" if Utils::is_blank?(item)
